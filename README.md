...
JavaScript是一门饱受争议的编程语言, 其存在固然有他优秀的一面. 当然, 在历史长河中得到的多数是批评, 这一点不可否认, 从它的某些"怪癖"确实能发现在语言设计上的缺陷和考虑的不周. 不过也恰恰因此使它看起来很神秘, Geek们喜欢通过Hack它来获取成就感, 比如`2 == [[[2]]]`返回`true`, `[][(([]+[])+![])[+!![]-(~(+!![]))]+(([]+[])+{})[+!![]]+(([]+[])+!![])[+!![]]+(([]+[])+!![])[+![]]]`等于函数`sort()`. 所以, 这里不管它是Good Parts还是Bad Parts, 先仔细玩味一番.
